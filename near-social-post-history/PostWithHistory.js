...
const nearDevGovGigsContractAccountId = props.nearDevGovGigsContractAccountId || "devgovgigs.near";
...
function markeljanHref(widgetName, linkProps) {
  ...
  const linkPropsQuery = Object.entries(linkProps)
    .map(([key, value]) => `${key}=${value}`)
    .join("&");
  return `#/markeljan.near/widget/${widgetName}${linkPropsQuery ? "?" : ""}${linkPropsQuery}`;
}

const postId = props.post.id ?? (props.id ? parseInt(props.id) : 0);
const post =
  props.post ?? Near.view(nearDevGovGigsContractAccountId, "get_post", { post_id: postId });
if (!post) {
...
const currentTimestampReadable = readableDate(Number(currentTimestamp) / 1000000);

const compareTimestampReadable = readableDate(Number(compareTimestamp) / 1000000);
      ...
      <div class="btn-group" role="group" aria-label="Basic outlined example">
        ...
        <div class="btn-group" role="group">
          ...
          <ul class="dropdown-menu">
            ...
            {btnCreatorWidget("Attestation", emptyIcons.Attestation, "Attestation")}
            {btnCreatorWidget("Sponsorship", emptyIcons.Sponsorship, "Sponsorship")}
            <li>
        ...
        >
          <i class="bi bi-arrows-expand"> </i> {`Expand Replies (${childPostIds.length})`}
        </button>
      </div>
    </div>
  </div>
...
const renamedPostType = snapshot.post_type == "Submission" ? "Solution" : snapshot.post_type;
...
function combineText(_snapshot) {
  return "## " + _snapshot.post_type + ": " + _snapshot.name + "\n" + _snapshot.description;
}
...
return (
  <Card className={`card my-2 ${borders[snapshot.post_type]}`}>
    ...
    <div className="card-body">
      {postLabels}
      {compareSnapshot ? (
        <div class="border rounded" style={{ marginTop: "16px", marginBottom: "16px" }}>
          ...
          <Widget
            ...
            props={{
              post: post,
              currentCode: combineText(swapTimestamps ? compareSnapshot : snapshot),
